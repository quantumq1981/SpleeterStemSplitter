...
class ChordAnalysis(models.Model):
    """Model representing chord analysis results for a source track or dynamic mix."""
    # UUID to uniquely identify analysis
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    # ID of the associated Celery task
    celery_id = models.UUIDField(default=None, null=True, blank=True)
    # Source track being analyzed
    source_track = models.ForeignKey(SourceTrack,
                                     related_name='chord_analyses',
                                     on_delete=models.CASCADE)
    # Optional: analyze from a DynamicMix (separated stems) for better accuracy
    dynamic_mix = models.ForeignKey(DynamicMix,
                                    related_name='chord_analyses',
    ...
    # Full chord analysis result (JSON)
    result_json = models.JSONField(default=dict, blank=True)
    # Simplified chord chart (Markdown)
    chart_markdown = models.TextField(blank=True)
    # Simplified chord chart (CSV)
    ...
    # Error message
    error = models.TextField(blank=True)
    # DateTime when analysis was started
    date_created = models.DateTimeField(auto_now_add=True)
    # DateTime when analysis completed/failed
    date_finished = models.DateTimeField(default=None, null=True, blank=True)
    ...
    def __str__(self):
        return f'Chord Analysis: {self.source_track} [{self.key}]'
