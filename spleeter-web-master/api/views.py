...
class ChordAnalysisCreateView(generics.ListCreateAPIView):
    """View for creating and listing chord analyses."""
    queryset = ChordAnalysis.objects.all()

    def get_serializer_class(self):
        if self.request.method == 'POST':
            return ChordAnalysisCreateSerializer
        return ChordAnalysisSerializer

    def create(self, request, *args, **kwargs):
        serializer = ChordAnalysisCreateSerializer(data=request.data)
        if not serializer.is_valid():
            return JsonResponse({
                'status': 'error',
    ...
    def perform_create(self, serializer):
        instance = serializer.save()
        result = analyze_chords.delay(str(instance.id))
        ChordAnalysis.objects.filter(id=instance.id).update(celery_id=result.id)


class ChordAnalysisRetrieveDestroyView(generics.RetrieveDestroyAPIView):
    """View for retrieving or deleting a chord analysis."""
    serializer_class = ChordAnalysisSerializer
    queryset = ChordAnalysis.objects.all()
    lookup_field = 'id'

    def delete(self, request, *args, **kwargs):
        analysis = self.get_object()
        if analysis.celery_id:
            app.control.revoke(analysis.celery_id, terminate=True, signal=KILL_SIGNAL)
        return super().destroy(request, *args, **kwargs)


class ChordAnalysisByTrackView(generics.ListAPIView):
    """View that lists chord analyses for a specific source track."""
    serializer_class = ChordAnalysisSerializer

    def get_queryset(self):
        track_id = self.kwargs['track_id']
        return ChordAnalysis.objects.filter(source_track_id=track_id)


class ChordAnalysisChartView(APIView):
    """View that returns the chord chart in a specific format."""
    def get(self, request, id):
        try:
            analysis = ChordAnalysis.objects.get(id=id)
        ...
        if fmt == 'csv':
            return JsonResponse({
                'format': 'csv',
                'chart': analysis.chart_csv,
            })
        elif fmt == 'markdown':
            return JsonResponse({
                'format': 'markdown',
                'chart': analysis.chart_markdown,
            })
        else:
            return JsonResponse({
                'format': 'json',
