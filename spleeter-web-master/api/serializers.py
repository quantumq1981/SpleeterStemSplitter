...
class ChordAnalysisSerializer(serializers.ModelSerializer):
    """Serializer for ChordAnalysis model."""
    status = ChoicesSerializerField()

    class Meta:
        model = ChordAnalysis
        fields = ('id', 'celery_id', 'source_track', 'dynamic_mix',
                  'key', 'key_confidence', 'result_json',
                  'chart_markdown', 'chart_csv',
                  'segment_duration', 'smoothing',
                  'status', 'error', 'date_created', 'date_finished')
        read_only_fields = ('id', 'celery_id', 'key', 'key_confidence',
                            'result_json', 'chart_markdown', 'chart_csv',
                            'status', 'error', 'date_created', 'date_finished')

class ChordAnalysisCreateSerializer(serializers.ModelSerializer):
    """Serializer for creating a new ChordAnalysis."""

    class Meta:
        model = ChordAnalysis
        fields = ('source_track', 'dynamic_mix', 'segment_duration', 'smoothing')

    def validate(self, data):
        if 'source_track' not in data:
            raise serializers.ValidationError(
                {'source_track': 'source_track is required.'})
        return data
